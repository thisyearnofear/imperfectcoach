...
async function testPaymentRequired() {
  ...
  try {
    ...
    if (response.statusCode === 402) {
      ...
      console.log(`  Amount: ${challenge.accepts[0].amount} ${challenge.accepts[0].currency}`);
      console.log(`  Chain ID: ${challenge.accepts[0].chainId}`);
      console.log(`  To Address: ${challenge.accepts[0].to}`);
      ...
      const isValid = (
        accept.scheme === 'usdc' &&
        accept.currency === 'USDC' &&
        accept.amount === '250000' &&
        accept.chainId === 84532 &&
        accept.to === '0x6C9BCfF8485B12fb8bd73B77638cd6b2dD0CF9CA' &&
      ...
      if (isValid) {
        ...
        console.log('  scheme: usdc');
        console.log('  currency: USDC');
        console.log('  amount: 250000');
        console.log('  chainId: 84532');
        console.log('  to: 0x6C9BCfF8485B12fb8bd73B77638cd6b2dD0CF9CA');
...
async function testEnvironmentConfig() {
  ...
  try {
    ...
    if (response.statusCode === 402) {
      ...
      console.log(`  ✓ Revenue Splitter Address: ${config.to}`);
      console.log(`  ✓ Base Sepolia Chain ID: ${config.chainId}`);
      console.log(`  ✓ USDC Payment Amount: ${config.amount} (0.25 USDC)`);
...
async function runTests() {
  ...
  if (passed === total) {
    ...
    console.log('❌ Some tests FAILED. Please check your Lambda configuration.');
    console.log('\nTroubleshooting:');
