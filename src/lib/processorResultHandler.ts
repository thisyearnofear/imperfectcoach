...
import {
    ProcessorResult, PoseData, RepState, WorkoutMode, Exercise,
    PullupRepDetails, JumpRepDetails, RepData
} from '@/lib/types';
    ...
    if (result.isRepCompleted && result.repCompletionData) {
        ...
        if (exercise === 'pull-ups') {
            ...
            if (left.length > 0 && right.length > 0) {
                ...
                const asymmetry = Math.max(...left.map((l, i) => Math.abs(l - safeRight[i])));
