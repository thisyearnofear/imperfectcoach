...
export type JumpState = 'GROUNDED' | 'PREPARING' | 'CROUCHED' | 'EXPLODING' | 'AIRBORNE' | 'LANDING';

interface PoseStability {
    anklePositions: CircularArray<{ left: { x: number, y: number }, right: { x: number, y: number } }>;
    shoulderPositions: CircularArray<{ left: { x: number, y: number }, right: { x: number, y: number } }>;
    hipPositions: CircularArray<{ left: { x: number, y: number }, right: { x: number, y: number } }>;
}
...
export class EnhancedJumpDetector {
    ...
    constructor(lag: number = 10, threshold: number = 1.5, influence: number = 0.3) {
        this.lag = lag;
        ...
        if (!requiredPoints) {
            ...
            if (this.jumpState !== 'GROUNDED' && this.lastValidPose) {
                ...
                if (fallbackPoints) {
                    ...
                    return {
                        ...result,
                        feedback: result.feedback + " (Pose detection unstable - try to stay visible)"
                    };
                }
            }
    ...
    private processJumpWithPoints(requiredPoints: ReturnType<typeof this.getRequiredKeypoints>) {
        if (!requiredPoints) {
            return {
                isValidJump: false,
                jumpState: this.jumpState,
        ...
        const { leftHip, rightHip, leftKnee, rightKnee, leftAnkle, rightAnkle, leftShoulder, rightShoulder } = requiredPoints;
        ...
        return this.processJumpState(avgKneeAngle, avgAnkleY, torsoY, statisticalSignal, requiredPoints);
    }
    ...
    private getRequiredKeypoints(keypoints: posedetection.Keypoint[]) {
        ...
        const corePoints = ['left_hip', 'right_hip', 'left_knee', 'right_knee', 'left_ankle', 'right_ankle'];
        const optionalPoints = ['left_shoulder', 'right_shoulder'];
        
        const found = Object.fromEntries(
            [...corePoints, ...optionalPoints].map(name => [name.replace('_', ''), keypoints.find(k => k.name === name)])
        );
        ...
        const leftShoulder = found.leftshoulder?.score > 0.3 ? found.leftshoulder : found.lefthip;
        const rightShoulder = found.rightshoulder?.score > 0.3 ? found.rightshoulder : found.righthip;
    ...
    private updatePoseStability(points: ReturnType<typeof this.getRequiredKeypoints>) {
        if (!points) return;
    ...
    private isPoseStable(): boolean {
        if (this.poseStability.anklePositions.length < this.STABILITY_REQUIRED_FRAMES) return false;
        ...
        const ankleMovement = this.calculateRecentMovement(this.poseStability.anklePositions);
        const hipMovement = this.calculateRecentMovement(this.poseStability.hipPositions);
        
        // Shoulders are now optional, so only check them if we have valid data
        const shoulderMovement = this.poseStability.shoulderPositions.length >= this.STABILITY_REQUIRED_FRAMES 
        ...
        const coreStable = ankleMovement < ANKLE_THRESHOLD && hipMovement < HIP_THRESHOLD;
        const shoulderStable = shoulderMovement === 0 || shoulderMovement < SHOULDER_THRESHOLD;
        
        // Debug logging to help understand why calibration fails
        if (process.env.NODE_ENV === 'development') {
            console.log(`📊 Stability Check - Ankle: ${ankleMovement.toFixed(1)}px (limit: ${ANKLE_THRESHOLD}), Hip: ${hipMovement.toFixed(1)}px (limit: ${HIP_THRESHOLD}), Shoulder: ${shoulderMovement.toFixed(1)}px (limit: ${SHOULDER_THRESHOLD}), Final Stable: ${coreStable && shoulderStable}`);
        }
        
        return coreStable && shoulderStable;
    }
    
    private calculateRecentMovement(positions: CircularArray<any>): number {
        ...
        for (let i = positions.length - recentFrames; i < positions.length - 1; i++) {
            const prev = positions[i];
            const curr = positions[i + 1];
            
            const leftMovement = Math.sqrt(
                Math.pow(curr.left.x - prev.left.x, 2) + Math.pow(curr.left.y - prev.left.y, 2)
            );
            const rightMovement = Math.sqrt(
                Math.pow(curr.right.x - prev.right.x, 2) + Math.pow(curr.right.y - prev.right.y, 2)
            );
    ...
    private calculateMaxMovement(positions: CircularArray<any>): number {
        if (positions.length < 2) return 0;
    ...
    private processJumpState(
        ...
        const currentHeight = this.groundLevel ? Math.max(0, this.groundLevel - avgAnkleY) : 0;
        
        // Debug logging for all blockers
        if (process.env.NODE_ENV === 'development') {
            console.log(`🔍 Calibration Debug:
                Jump State: ${this.jumpState}
                Knee Angle: ${avgKneeAngle.toFixed(1)}° (need: ${this.MIN_CROUCH_ANGLE}°+)
                Is Stable: ${isStable}
                Stable Frames: ${this.stableFrameCount}/${this.STABILITY_REQUIRED_FRAMES}
                Required Points Available: ${!!points}
                All Conditions Met: ${avgKneeAngle > this.MIN_CROUCH_ANGLE && isStable}
            `);
        }
        ...
        switch (this.jumpState) {
                ...
                if (avgKneeAngle > this.MIN_CROUCH_ANGLE && isStable) {
                    ...
                    if (this.stableFrameCount >= this.STABILITY_REQUIRED_FRAMES) {
                        ...
                        return {
                            isValidJump: false,
                            jumpState: this.jumpState,
                            feedback: "Perfect! Now crouch down and explode upward for your jump!",
                            shouldCompleteRep: false,
                            currentHeight: 0,
                            calibrationData: {
                        ...
                        const progress = Math.round((this.stableFrameCount / this.STABILITY_REQUIRED_FRAMES) * 100);
                        return {
                            isValidJump: false,
                            jumpState: this.jumpState,
                    ...
                    return {
                        ...
                        currentHeight: 0,
                        calibrationData: {
                            calibrationProgress: Math.max(0, (this.stableFrameCount / this.STABILITY_REQUIRED_FRAMES) * 100),
                            isStable: isStable,
                            kneeAngle: avgKneeAngle,
                            minKneeAngle: this.MIN_CROUCH_ANGLE,
                            isCalibrating: true
            ...
            case 'EXPLODING':
                if (currentHeight > 10) { // Minimum height for valid jump
            ...
            case 'AIRBORNE':
                if (!isStatisticallyAirborne && currentHeight < 8) {
                    ...
                    return {
                        isValidJump: true,
                        jumpState: this.jumpState,
                        feedback: avgKneeAngle < this.LANDING_DETECTION_ANGLE ? "Excellent soft landing!" : "Bend your knees more on landing!",
    ...
    private updateStatisticalDetector(newValue: number): number {
        ...
        if (std > 0 && Math.abs(newValue - mean) > this.threshold * std) {
            ...
            const influencedValue = this.influence * newValue + (1 - this.influence) * (this.filteredY[this.filteredY.length - 1] || newValue);
